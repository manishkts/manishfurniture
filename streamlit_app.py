...
# Helper function to load data safely and prevent column mismatches
def load_data(file_name, expected_columns):
...
# Load clean, verified data structures
if "workers" not in st.session_state:
    st.session_state.workers = load_data("workers.csv", WORKER_COLS)
if "logs" not in st.session_state:
    st.session_state.logs = load_data("logs.csv", LOG_COLS)
if "financials" not in st.session_state:
    st.session_state.financials = load_data("financials.csv", FIN_COLS)
    ...
    with col_add:
        ...
        with st.form("Add Worker Form", clear_on_submit=True):
            ...
            if submit_worker and w_name:
                new_worker = pd.DataFrame([[w_id, w_name, w_phone, w_skill]], columns=WORKER_COLS)
    ...
    with col_del:
        ...
        if st.session_state.workers.empty:
            ...
            if st.button("❌ Delete Selected Worker", type="primary"):
                ...
                save_data(st.session_state.workers, "workers.csv")
                st.success(f"Removed worker record!")
                st.rerun()
    ...
    if st.session_state.logs.empty:
        ...
        with col_del:
            ...
            if st.session_state.financials.empty:
                ...
                fin_list = st.session_state.financials["Payment ID"].astype(str) + " (Log Ref: " + st.session_state.financials["Log ID"].astype(str) + ")"
                ...
                if st.button("❌ Delete Selected Financial Record", type="primary"):
                    st.session_state.financials = st.session_state.financials[st.session_state.financials["Payment ID"] != delete_p_id]
                    save_data(st.session_state.financials, "financials.csv")
